    ...
    {   
        ...
        {   
            ...
            for(unsigned int j = 0; j<PIPE_WIDTH; j++)
            {
                ...
                {
                    //std::cout<<"p->pipe_latch[EX_LATCH][j].trace_rec.dest_needed: "<<(int)p->pipe_latch[EX_LATCH][j].trace_rec.dest_needed<<std::endl;
                    ...
                    if(p->pipe_latch[EX_LATCH][j].trace_rec.dest_needed==1)
                    {
                        ...
                        {
                            //std::cout<<"Hazard 1 scr1 EX stage"<<std::endl;
                            //std::cout<<"Done ID Funciton"<<std::endl;
                            p->pipe_latch[ID_LATCH][i].stall = 1;
                        }
                        else if(p->pipe_latch[ID_LATCH][i].trace_rec.src2_needed)
                        {
                            ...
                            p->pipe_latch[EX_LATCH][j].trace_rec.dest_reg )
                            {
                                //std::cout<<"Hazard 1 scr2 EX stage"<<std::endl;
                                //std::cout<<"Done ID Funciton"<<std::endl;
                                p->pipe_latch[ID_LATCH][i].stall = 1;
                                //std::cout<<"ID Stall Signal: "<<p->pipe_latch[ID_LATCH][i].stall<<std::endl;
                            }
                        }
                    }
                }
