...
void pipe_check_bpred(Pipeline *p, PipelineLatch *fetch_op)
{
    ...
    if (fetch_op->op_id == OP_CBR)
    {
        // TODO: For a conditional branch instruction, get a prediction from the
        // branch predictor.
        BPred Check(BPRED_POLICY);
        BranchDirection takenChoice = Check.predict(fetch_op->op_id);
                
        // TODO: If the branch predictor mispredicted, mark the fetch_op
        // accordingly.        
        if (takenChoice != const_cast<BranchDirection>(fetch_op->trace_rec.br_dir))
        {
        ...
        Check.update(fetch_op->op_id, takenChoice);
