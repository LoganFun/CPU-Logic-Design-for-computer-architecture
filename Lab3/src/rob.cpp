...
int rob_insert(ROB *rob, InstInfo inst)
{
    ...
    {        
        ...
        //initialize Signal
        if (rob->entries[rob->tail_ptr].inst.src1_reg==-1)
        {
            rob->entries[rob->tail_ptr].inst.src1_ready=true;
        }
        if (rob->entries[rob->tail_ptr].inst.src2_reg==-1)
        {
            rob->entries[rob->tail_ptr].inst.src2_ready=true;
        }
        ...
        return_value = rob->tail_ptr;

        rob->tail_ptr++;
        if(uint32_t(rob->tail_ptr) >= NUM_ROB_ENTRIES)
            rob->tail_ptr -= NUM_ROB_ENTRIES;
...
void rob_mark_exec(ROB *rob, InstInfo inst)
{
    ...
    // I guess is the dr_tag
    // TODO: Update that entry.
    for (uint32_t i = 0; i < NUM_ROB_ENTRIES; i++)
    {
        if (inst.inst_num==rob->entries[i].inst.inst_num)
        {
            //printf("test 1 for ins num value: %lu",rob->entries[i].inst.inst_num);    
 ...
 * 
 * @param rob the ROB
 * @param inst the instructioSn whose output is ready
 */
void rob_mark_ready(ROB *rob, InstInfo inst)
{
    // TODO: Find the entry in the ROB containing the given instruction.
...
void rob_wakeup(ROB *rob, int tag)
{
    ...
    for (uint32_t i = 0; i < NUM_ROB_ENTRIES; i++)
    {
        //check whether the ins is valid
...
InstInfo rob_remove_head(ROB *rob)
{
    ...
    InstInfo return_Value;
    //if (rob->entries[rob->head_ptr].ready == true)
    //{
        
        return_Value = rob->entries[rob->head_ptr].inst;
        rob->entries[rob->head_ptr].valid=false;
        rob->entries[rob->head_ptr].exec=false;
        rob->entries[rob->head_ptr].ready=false;
        rob->head_ptr++;
        //wrap around
        if(uint32_t(rob->head_ptr)>=NUM_ROB_ENTRIES)
            rob->head_ptr-=NUM_ROB_ENTRIES;
        return return_Value;
    //} 
}
